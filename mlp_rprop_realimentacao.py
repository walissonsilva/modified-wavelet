...
class MLP_R(object):
    def __init__(self, delta0=0.1, delta_max=0.1, delta_min=1e-10, epoch_max=20000, Ni=4, Nh=6, Ns=1):
        self.delta0 = delta0
    ...
    def train(self):
        self.Wji = np.random.normal(self.Wini, self.Wini / 4, (self.Nh, self.Ni))#np.random.rand(self.Nh, self.Ni) * self.Wini
        self.Wkj = np.random.normal(self.Wini, self.Wini / 4, (self.Ns, self.Nh + 2))#np.random.rand(self.Ns, self.Nh + 1) * self.Wini
        self.taWji = np.ones((self.Nh, self.Ni)) * self.delta0
        self.taWkj = np.ones((self.Ns, self.Nh + 2)) * self.delta0
        grji_ant = grkj_ant = 0
        ...
        for epoca in xrange(self.epoch_max):
            ...
            if (epoca == 0):
                for i in xrange(self.N):
                    xi = np.array([-1, self.X_train[i], self.X_train[i]**2, self.X_train[i]**4]).reshape(1, -1)
                    netj = np.dot(self.Wji, xi.T)
                    #yj = np.tanh(netj.T)
                    yj = 1 / (1 + np.exp(-netj.T))
                    yj_pol = np.insert(yj[0], 0, -1).reshape(1, -1)
                    yj_full = np.concatenate(([-1], yj[0], [0])).reshape(1, -1)
                    z[i] = np.dot(self.Wkj, yj_full.T)[0][0]

                    e = self.d[i] - z[i]

                    gradkj += -e * np.insert(yj_pol, -1, z[i]).reshape(1, -1)
                    gradji += np.dot((-e * self.Wkj[:,1:(self.Nh+1)] * (yj * (1 - yj))).T, xi)
                ...
                for i in xrange(self.N):
                    ...
                    yj = 1 / (1 + np.exp(-netj.T))
                    yj_pol = np.insert(yj[0], 0, -1).reshape(1, -1)
                    yj_full = np.concatenate(([-1], yj[0], [zo[i]])).reshape(1, -1)
                    z[i] = np.dot(self.Wkj, yj_full.T)[0][0]
                    look = zo[i] - z[i]

                    e = self.d[i] - z[i] + look

                    gradkj += -e * np.insert(yj_pol, -1, zo[i]).reshape(1, -1)
                    gradji += np.dot((-e * self.Wkj[:,1:(self.Nh+1)] * (yj * (1 - yj))).T, xi)
